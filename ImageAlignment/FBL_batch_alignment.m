file=dir('try_result/*.tif');
f=im2double(imread(strcat('try_result/',file(1).name)));
g=im2double(imread(strcat('try_result/',file(2).name)));
usfac = 100;
merr=zeros(1,360);
si=size(f);
...
[mi,idx]=min(merr);
total=(f+mGreg(:,:,idx))/2;
imwrite(mGreg(:,:,idx),'FBL_try_register/reg_1.tif');
imwrite(total,'FBL_try_ref/ref_1.tif');
for n=3:length(file)
    f=total;
