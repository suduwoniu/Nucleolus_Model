...
%random choose processing factors position in the field
count=count-1;
p=randperm(count);
p1=p(1:nprocess);
...
%load('point_12.mat');
%randomFactor(:,2:4)=point;
%equally choose the inner ball-like pre-rRNA positions 
count0=0;
for xx=18:33
    for yy=18:33
        for zz=18:33
...
plot3(factor_record(:,1,cn),factor_record(:,2,cn),factor_record(:,3,cn),'.');
plot3(factor_record(:,1,cn),factor_record(:,2,cn),factor_record(:,3,cn),'.');
save('inner3_calo_meto/random_v4_s3_4_500_500_res.mat','randomFactor_res');
save('inner3_calo_meto/radom_v4__s3_4_500_500.mat','factor_record');
